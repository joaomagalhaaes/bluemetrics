...
export async function GET(req: NextRequest) {
  try {
    ...
    if (sinceParam && untilParam) {
      ...
      const start = new Date(`${nowBR.getFullYear()}-${String(nowBR.getMonth()).padStart(2,'0')}-01T00:00:00-03:00`)
      const endDate = new Date(nowBR.getFullYear(), nowBR.getMonth(), 0)
      const end = new Date(`${endDate.getFullYear()}-${String(endDate.getMonth()+1).padStart(2,'0')}-${String(endDate.getDate()).padStart(2,'0')}T23:59:59.999-03:00`)
      dateFilter = { date: { gte: start, lte: end } }
    } else if (period === 'week') {
